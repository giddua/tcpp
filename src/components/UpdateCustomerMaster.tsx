...
export default function UpdateCustomerMaster() {
  ...
  const fetchLastUpdate = async () => {
    try {
      const res = await fetch('/api/customer-master/last-update');
      if (res.ok) {
  ...
  const fetchSyncStatus = async () => {
    try {
      const res = await fetch('/api/customer-master/sync-status');
      if (res.ok) {
  ...
  const handleStartSync = async () => {
    ...
    try {
      const response = await fetch('/api/customer-master/sync', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="p-8 flex-1 max-w-7xl mx-auto w-full" id="customer-master-sync-root">
      {/* Breadcrumb & Title */}
      <div className="mb-8">
        <nav className="flex text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">
          ...
          <span className="text-[#003461]">Update Customer Master</span>
        </nav>
        <h2 className="text-3xl font-extrabold tracking-tight text-[#003461] dark:text-blue-400 flex items-center">
          Update Customer Master
        </h2>
        <p className="text-slate-500 mt-2 text-sm max-w-2xl">
          Execute background data synchronization pipelines. Pull alpha customer listings directly from Snowflake and batch overwrite ref.CustomerMaster structures in SQL Server.
        </p>
      </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-800">
            ...
            <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-lg flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 md:space-x-4 mb-6 border border-slate-100 dark:border-slate-800">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-mono mt-0.5">ref.CustomerMaster</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4 mb-6 text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
              <div className="flex items-start space-x-2">
                ...
                <p>
                  This execution fetches all unique <strong>CustomerId (address_number)</strong> and <strong>CustomerName (NAME_ALPHA)</strong> pairings.
                </p>
              </div>
              <div className="flex items-start space-x-2">
                <Info className="w-4 h-4 text-blue-500 shrink-0 mt-0.5" />
                <p>
                  Because this returns approximately <strong>55,000 records</strong>, queries are retrieved in a background job and inserted into SQL Server in batch packets to prevent timeouts.
                </p>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl p-6 shadow-sm border border-slate-200 dark:border-slate-800">
            ...
            <div className="space-y-6">
              <div className="pb-6 border-b border-slate-100 dark:border-slate-800">
                <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider mb-2">Customer Master Last Sync</p>
                {lastUpdated ? (
              ...
              <div>
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <span className="font-mono text-[11px] font-bold text-slate-700 bg-slate-50 dark:bg-slate-950 px-2 py-0.5 rounded">CustomerMaster</span>
                  </div>
                  <div className="flex justify-between items-center text-xs">
