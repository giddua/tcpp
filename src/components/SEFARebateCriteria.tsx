...
type SortField = keyof SEFARebateCriteria;
type SortOrder = 'asc' | 'desc';

export default function SEFARebateCriteria() {
  const [criteria, setCriteria] = useState<SEFARebateCriteria[]>([]);
  ...
  const [editingCriteria, setEditingCriteria] = useState<SEFARebateCriteria | null>(null);
  ...
  const fetchCriteria = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/sefa-rebate-criteria');
      const data = await response.json();
      if (response.ok) {
        setCriteria(data);
      } else {
        setError(data.details || data.error || 'Failed to fetch SEFA rebate criteria');
      }
    } catch (err: any) {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch('/api/sefa-product-categories');
      const data = await response.json();
      if (response.ok) {
        setCategories(data);
      }
  ...
  const filteredAndSortedCriteria = useMemo(() => {
    return criteria
      .filter(c => 
        ...
        (c.ExclusionReason?.toLowerCase().includes(searchTerm.toLowerCase()) || false)
      )
      .sort((a, b) => {
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!window.confirm('Are you sure you want to delete this SEFA rebate criteria?')) return;

    try {
      const response = await fetch(`/api/sefa-rebate-criteria/${id}`, { method: 'DELETE' });
      const result = await response.json();
      if (response.ok) {
        setSuccess('SEFA rebate criteria deleted successfully');
        fetchCriteria();
      } else {
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const payload = {
      ...
      TargetThresholdPercent: formData.get('TargetThresholdPercent'),
      ExclusionReason: formData.get('ExclusionReason'),
    };

    const url = editingCriteria ? `/api/sefa-rebate-criteria/${editingCriteria.SS_ID}` : '/api/sefa-rebate-criteria';
  ...
  const openModal = (c: SEFARebateCriteria | null = null) => {
      ...
      <div className="mb-8 flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-500 mt-2 text-sm max-w-2xl">
            Manage product categories and inclusion criteria for year-over-year SEFA rebate calculations. These rules are specific to the 'SEFA_YOY' screen category.
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800">
                ...
                <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-slate-500 cursor-pointer text-center" onClick={() => handleSort('TargetThresholdPercent')}>
                   <div className="flex items-center justify-center">Threshold % <SortIcon field="TargetThresholdPercent" /></div>
                </th>
                <th className="px-6 py-4 text-[10px] font-bold uppercase tracking-widest text-slate-500">Reason/Criteria</th>
              ...
              {isLoading ? (
                ...
                filteredAndSortedCriteria.map((c) => (
                  <tr key={c.SS_ID} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className="text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded border border-amber-100">{c.TargetThresholdPercent}%</span>
                    </td>
                    <td className="px-6 py-4">
                      <p className="text-[11px] text-slate-500 line-clamp-1 italic">{c.ExclusionReason || '---'}</p>
                    </td>
                    <td className="px-6 py-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="bg-white w-full max-w-xl rounded-2xl shadow-2xl overflow-hidden relative z-10 border border-slate-200">
              <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
                <h3 className="text-sm font-bold text-[#003461] uppercase tracking-widest flex items-center">
                  ...
                  {editingCriteria ? 'Edit SEFA Rebate Criteria' : 'New SEFA Criteria'}
                </h3>
                ...
                <div className="p-6 space-y-6 overflow-y-auto flex-1 text-slate-800">
                  ...
                  <div className="space-y-5">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 block">
                          Target Threshold % <span className="text-red-500 ml-1">*</span>
                        </label>
                        <input 
                          name="TargetThresholdPercent"
                          type="number"
                          step="0.01"
                          min="0"
                          max="100"
                          defaultValue={editingCriteria?.TargetThresholdPercent ?? 0}
                          className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none" 
                          placeholder="0.00"
                          required
                        />
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center space-x-1 text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1">
                        ...
                        <label>Reason / Criteria Note</label>
                      </div>
                      <textarea 
                        name="ExclusionReason"
                        rows={3}
                        defaultValue={editingCriteria?.ExclusionReason || ''}
                        ...
                        placeholder="Detail the criteria or reason..."
                      />
                    </div>
                  </div>
