...
export default function USScorecardCustomer() {
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      ...
      setError(null);
      try {
        const response = await fetch('/api/reports/scorecard/customers');
        if (!response.ok) {
        ...
        console.error('Error fetching scorecard customers:', err);
  ...
  const handleDownload = async () => {
    ...
    try {
      const response = await fetch('/api/reports/scorecard/export', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        const errJson = await response.json().catch(() => ({}));
        throw new Error(errJson.details || errJson.error || 'Failed to export scorecard spreadsheet.');
      }
    ...
    <div className="flex-1 p-8 overflow-y-auto">
      ...
      <div className="mb-8">
        <nav className="text-sm font-medium text-slate-500 mb-2">
          Reports / <span className="text-[#003461] font-semibold">US Scorecard by Customer</span>
        </nav>
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-3xl font-extrabold tracking-tight text-[#003461] dark:text-blue-400">US Scorecard by Customer</h2>
            <p className="text-slate-500 mt-1 max-w-3xl text-sm">
              Generate localized scorecard worksheets from Snowflake dynamically. Select a distinct customer name to overlay rows onto the official template.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-4 flex items-center">
              ...
              Snowflake Customer Lookup
            </h3>

            {loading ? (
              <div className="py-12 flex flex-col items-center justify-center space-y-3">
                <Loader2 className="w-8 h-8 text-[#003461] animate-spin" />
                <p className="text-sm font-medium text-slate-500">Querying distinct parent names from Snowflake...</p>
              </div>
            ) : error ? (
              <div className="mb-6">
            ...
            {!loading && (
              <div className="space-y-4">
                ...
                <p className="text-[11px] text-slate-400">
                  This lists distinct customer values fetched live from PRD_REPORTING_LAYER.TCPP_SCORECARD_US_PIVOT.
                </p>
              </div>
            )}
          </div>
            ...
            {selectedCustomer ? (
              <div className="space-y-6">
                ...
                <div className="space-y-3 text-xs">
                  <div className="flex justify-between py-2 border-b border-slate-100">
                    ...
                    <span className="font-semibold text-slate-800">Snowflake</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-slate-100">
                    ...
                    <span className="font-mono text-[10px] text-slate-600 font-semibold bg-slate-50 px-1.5 py-0.5 rounded border border-slate-100">
                      TIERED_REBATES_SCORECARD_US_Template.xlsm
                    </span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-slate-100">
              ...
              <div className="py-12 px-4 text-center rounded-lg border border-dashed border-slate-200">
                ...
                <p className="text-xs text-slate-400 max-w-[200px] mx-auto mt-1">
                  Select a parent customer name from the dropdown on the left to activate download configurations.
                </p>
              </div>
            )}
          </div>
