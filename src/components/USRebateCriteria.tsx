...
type SortField = keyof USRebateCriteria;
type SortOrder = 'asc' | 'desc';

export default function USRebateCriteria() {
  const [criteria, setCriteria] = useState<USRebateCriteria[]>([]);
  ...
  const [editingCriteria, setEditingCriteria] = useState<USRebateCriteria | null>(null);
  ...
  const fetchCriteria = async () => {
    setError(null);
    try {
      const response = await fetch('/api/us-rebate-criteria');
      const data = await response.json();
      if (response.ok) {
        setCriteria(data);
      } else {
        setError(data.details || data.error || 'Failed to fetch US rebate criteria');
      }
    } catch (err: any) {
  ...
  const fetchCategories = async () => {
    try {
      const response = await fetch('/api/us-product-categories');
      const data = await response.json();
      if (response.ok) {
        setCategories(data);
      }
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!window.confirm('Are you sure you want to delete this US rebate criteria?')) return;

    try {
      const response = await fetch(`/api/us-rebate-criteria/${id}`, { method: 'DELETE' });
      const result = await response.json();
      if (response.ok) {
        setSuccess('US rebate criteria deleted successfully');
        fetchCriteria();
      } else {
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const url = editingCriteria ? `/api/us-rebate-criteria/${editingCriteria.SS_ID}` : '/api/us-rebate-criteria';
  ...
  const openModal = (c: USRebateCriteria | null = null) => {
      ...
      <div className="mb-8 flex justify-between items-end">
        <div>
          <nav className="flex text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">
            ...
            <span className="text-[#003461]">US Rebate Criteria</span>
          </nav>
          ...
          <p className="text-slate-500 mt-2 text-sm max-w-2xl">
            Manage product categories and threshold criteria for US rebate calculations. Rules are specific to the 'UDC_57_TT_US' category.
          </p>
        </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="bg-white w-full max-w-xl rounded-2xl shadow-2xl overflow-hidden relative z-10 border border-slate-200">
              <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
                <h3 className="text-sm font-bold text-[#003461] uppercase tracking-widest flex items-center">
                  ...
                  {editingCriteria ? 'Edit US Rebate Criteria' : 'New US Criteria'}
                </h3>
